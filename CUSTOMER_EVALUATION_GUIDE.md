...
links, live Project node IDs, and source issue/PR history references. It denies
non-UTF-8 files except the exact bounded authority-walkthrough GIF path, whose
signature, dimensions, size, terminator, and lack of GIF comment extensions are
validated together with its reviewed SHA-256 digest. Deterministic tests
separately require that recording to equal the executable walkthrough's
generated bytes and complete static transcript.
