---
name: Agentic framing
description: Stages a bounded framing comment from an authorized default-branch slash command.
on:
  slash_command:
    name: agentic-frame
  ...
  steps:
    ...
    - name: Authorize framing before model execution
      ...
      env:
        ...
        RUNTIME_ROLE: framer
        RUNTIME_CAPABILITY: core.frame-artifact@1.0.0
        RUNTIME_WORKFLOW_ID: agentic-framing
...
engine:
  id: copilot
  agent: runtime-framer
  max-continuations: 1
  concurrency:
    group: gh-aw-copilot-agentic-framing
network: {}
tools:
  github:
    toolsets: [issues]
...
concurrency:
  group: agentic-framing-${{ github.repository_id }}-${{ github.event.issue.number }}
...
Use the `bounded-framing` skill. Produce one target-free advisory comment with:
