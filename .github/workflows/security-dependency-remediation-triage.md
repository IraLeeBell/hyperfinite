...
on:
  slash_command:
    name: security-dependency-remediation-triage
  ...
  steps:
    ...
    - name: Authorize framing before model execution
      ...
      env:
        ...
        RUNTIME_ROLE: framer
        RUNTIME_CAPABILITY: demo.security-dependency-remediation.triage@1.0.0
        RUNTIME_WORKFLOW_ID: security-dependency-remediation-triage
        RUNTIME_DEMO_PROJECT_ID: security-dependency-remediation
        RUNTIME_STAGE_ID: triage
...
engine:
  id: copilot
  agent: security-dependency-remediation-triage
  max-continuations: 1
  concurrency:
    group: gh-aw-copilot-security-dependency-remediation-triage
network: {}
tools:
  github:
    toolsets: [issues]
...
skills:
  - .github/skills/security-dependency-remediation-triage
...
Use the `security-dependency-remediation-triage` skill. Produce one target-free advisory comment with:
