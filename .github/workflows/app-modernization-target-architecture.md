---
name: App Modernization target architecture
description: Stages a bounded target architecture comment from the reserved App Modernization stage.
on:
  slash_command:
    name: app-modernization-target-architecture
  ...
  steps:
    ...
    - name: Build the deterministic guard
      if: steps.check_command_position.outputs.command_position_ok == 'true' && steps.check_membership.outputs.is_team_member == 'true'
      run: npm run build
    - name: Authorize target architecture before model execution
      id: trusted_guard
      if: steps.check_command_position.outputs.command_position_ok == 'true' && steps.check_membership.outputs.is_team_member == 'true'
      env:
        ...
        RUNTIME_ROLE: framer
        RUNTIME_CAPABILITY: demo.app-modernization.target-architecture@1.0.0
        RUNTIME_WORKFLOW_ID: app-modernization-target-architecture
        RUNTIME_DEMO_PROJECT_ID: app-modernization
        RUNTIME_STAGE_ID: target-architecture
...
engine:
  id: copilot
  agent: app-modernization-target-architecture
  max-continuations: 1
  concurrency:
    group: gh-aw-copilot-app-modernization-target-architecture
network: {}
tools:
  github:
    toolsets: [issues]
...
skills:
  - .github/skills/app-modernization-target-architecture
...
# Authorized App Modernization target architecture

Read only issue `${{ github.event.issue.number }}` in the current repository `${{ github.repository }}` and the trusted activation context. Use `app-modernization-target-architecture` and `authority-refusal`.

Propose only target-free principles, component responsibilities, interfaces, decisions, and non-goals from the current assessment and risk evidence. Preserve behavior and repository-only delivery. Never select a repository, path, route, transition, capability, tool, credential, retry, effect, deployment, or publication target, and never claim approval, merge, or completion.
