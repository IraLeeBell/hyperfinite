...
description: Stages a target-free bounded patch from an authorized issue command.
on:
  slash_command:
  ...
  steps:
    ...
    - name: Authorize execution before model execution
      ...
      env:
        ...
        RUNTIME_CAPABILITY: core.execute-bounded-change@1.0.0
        RUNTIME_WORKFLOW_ID: agentic-execution
    ...
    - name: Transfer the trusted execution authorization
      ...
      with:
        name: agentic-execution-authorization-${{ github.run_id }}-${{ github.run_attempt }}
...
engine:
  ...
  max-continuations: 1
  concurrency:
    group: gh-aw-copilot-agentic-execution
network: {}
tools:
  github: false
skills:
  - .github/skills/bounded-execution
...
concurrency:
  group: agentic-execution-${{ github.repository_id }}-${{ github.event.issue.number }}
    ...
    stage-implementation-patch:
      ...
      steps:
        ...
        - name: Transfer the signed trusted execution bundle
          ...
          uses: actions/upload-artifact@v7
          with:
            name: agentic-execution-bundle-${{ github.run_id }}-${{ github.run_attempt }}
        ...
        - name: Invoke the trusted execution delivery service
          ...
          env:
            ...
            TRUSTED_EXECUTION_ARTIFACT_NAME: agentic-execution-bundle-${{ github.run_id }}-${{ github.run_attempt }}
