---
name: Security remediation patch implementation
...
on:
  ...
  steps:
    ...
    - name: Build the deterministic guard
      if: steps.check_command_position.outputs.command_position_ok == 'true' && steps.check_membership.outputs.is_team_member == 'true'
      run: npm run build
    - name: Authorize execution before model execution
      id: trusted_guard
      if: steps.check_command_position.outputs.command_position_ok == 'true' && steps.check_membership.outputs.is_team_member == 'true'
      env:
        ...
        RUNTIME_CAPABILITY: demo.security-dependency-remediation.patch-implementation@1.0.0
        RUNTIME_WORKFLOW_ID: security-dependency-remediation-patch-implementation
        RUNTIME_DEMO_PROJECT_ID: security-dependency-remediation
        RUNTIME_STAGE_ID: patch-implementation
    ...
    - name: Transfer the trusted execution authorization
      ...
      with:
        name: security-dependency-remediation-patch-implementation-authorization-${{ github.run_id }}-${{ github.run_attempt }}
...
engine:
  id: copilot
  agent: security-dependency-remediation-patch-implementation
  max-continuations: 1
  concurrency:
    group: gh-aw-copilot-security-dependency-remediation-patch-implementation
network: {}
tools:
  github: false
skills:
  - .github/skills/security-dependency-remediation-patch-implementation
...
safe-outputs:
  ...
  jobs:
    stage-implementation-patch:
      description: Validate one closed target-free implementation patch and stage its trusted delivery handoff.
      runs-on: ubuntu-slim
      ...
      steps:
        ...
        - name: Transfer the signed trusted execution bundle
          ...
          uses: actions/upload-artifact@v7
          with:
            name: security-dependency-remediation-patch-implementation-bundle-${{ github.run_id }}-${{ github.run_attempt }}
        ...
        - name: Invoke the trusted execution delivery service
          ...
          env:
            ...
            TRUSTED_EXECUTION_ARTIFACT_NAME: security-dependency-remediation-patch-implementation-bundle-${{ github.run_id }}-${{ github.run_attempt }}
...
Use `security-dependency-remediation-patch-implementation` and `authority-refusal`. Call `stage_implementation_patch` exactly once with the signed planning and execution-grant digests plus one `TargetFreePatch@1.0.0` JSON value. The patch may contain only the authorized logical slots and UTF-8 file content. Trusted code maps slots to exact repository paths, creates the isolated worktree, validates the diff, runs every reviewed command ID, and stages the handoff consumed by the Control Kernel and GitHub adapter.

Do not select or emit repository paths, branches, commits, pull requests, Project items, routes, capabilities, credentials, retries, or effects. Do not use network, MCP, shell, or secrets. Do not approve, merge, deploy, publish, or claim completion. The runtime is disabled by default and this workflow has no GitHub tool access or live safe-output mutation.
