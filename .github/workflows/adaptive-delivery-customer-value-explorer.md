---
name: Adaptive Delivery Customer Value Explorer
description: Produces bounded discovery - customer value explorer advisory evidence for one governed selection.
on:
  slash_command:
    name: adaptive-delivery-customer-value-explorer
  ...
  steps:
    ...
    - name: Authorize solution design before model execution
      ...
      env:
        ...
        RUNTIME_CAPABILITY: demo.adaptive-delivery.customer-value-explorer@1.0.0
        RUNTIME_WORKFLOW_ID: adaptive-delivery-customer-value-explorer
...
engine:
  id: copilot
  agent: adaptive-delivery-customer-value-explorer
  max-continuations: 1
  concurrency:
    group: gh-aw-copilot-adaptive-delivery-customer-value-explorer
network: {}
tools:
  github:
    toolsets: [issues]
...
skills:
  - .github/skills/adaptive-delivery-customer-value-explorer
...
Use `adaptive-delivery-customer-value-explorer` and `authority-refusal`. Produce one target-free design decision traced to every stable acceptance criterion and only the trusted logical slots in the activation context.
