  ...
  steps:
    ...
    - name: Authorize solution design before model execution
      ...
      env:
        ...
        RUNTIME_STAGE_ID: context-inventory
        RUNTIME_CAPABILITY: demo.adaptive-delivery.context-inventory@1.0.0
        RUNTIME_WORKFLOW_ID: adaptive-delivery-context-inventory
...
engine:
  id: copilot
  agent: adaptive-delivery-context-inventory
  max-continuations: 1
  concurrency:
    group: gh-aw-copilot-adaptive-delivery-context-inventory
network: {}
tools:
  github:
    toolsets: [issues]
...
skills:
  - .github/skills/adaptive-delivery-context-inventory
...
Use `adaptive-delivery-context-inventory` and `authority-refusal`. Produce one target-free design decision traced to every stable acceptance criterion and only the trusted logical slots in the activation context.
