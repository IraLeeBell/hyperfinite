...
on:
  ...
  steps:
    ...
    - name: Build the deterministic guard
      if: steps.check_command_position.outputs.command_position_ok == 'true' && steps.check_membership.outputs.is_team_member == 'true'
      run: npm run build
    - name: Authorize framing before model execution
      id: trusted_guard
      if: steps.check_command_position.outputs.command_position_ok == 'true' && steps.check_membership.outputs.is_team_member == 'true'
      env:
        ...
        RUNTIME_ROLE: framer
        RUNTIME_CAPABILITY: demo.security-dependency-remediation.reproduction-and-impact-analysis@1.0.0
        RUNTIME_WORKFLOW_ID: security-dependency-remediation-reproduction-and-impact-analysis
        RUNTIME_DEMO_PROJECT_ID: security-dependency-remediation
        RUNTIME_STAGE_ID: reproduction-and-impact-analysis
...
engine:
  id: copilot
  agent: security-dependency-remediation-reproduction-and-impact-analysis
  max-continuations: 1
  concurrency:
    group: gh-aw-copilot-security-dependency-remediation-reproduction-and-impact-analysis
network: {}
tools:
  github:
    toolsets: [issues]
...
skills:
  - .github/skills/security-dependency-remediation-reproduction-and-impact-analysis
...
Read only issue `${{ github.event.issue.number }}` in the current repository `${{ github.repository }}` and the activation context.

The trusted pre-activation job redeemed authorization `${{ needs.pre_activation.outputs.authorization_digest }}` with redemption `${{ needs.pre_activation.outputs.redemption_digest }}` for this exact run.

Use the `security-dependency-remediation-reproduction-and-impact-analysis` skill. Produce one target-free advisory comment with:

- a concise objective;
- in-scope and excluded work;
- evidence-backed assumptions and dependencies;
- unresolved questions;
- explicit escalation when state, policy, evidence, or authority is missing.

Do not choose a target, route, transition, capability, tool, credential, or effect. Do not claim approval, merge, deployment, publication, or completion authority. The configured safe output is staged; the trusted runtime bridge alone can translate exact-success evidence into a bound comment plan.
