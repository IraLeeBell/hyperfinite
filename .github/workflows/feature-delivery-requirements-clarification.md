...
description: Stages a bounded feature brief from an authorized default-branch command.
on:
  slash_command:
    name: feature-delivery-requirements-clarification
  ...
  steps:
    ...
    - name: Build the deterministic guard
      if: steps.check_command_position.outputs.command_position_ok == 'true' && steps.check_membership.outputs.is_team_member == 'true'
      run: npm run build
    - name: Authorize requirements clarification before model execution
      id: trusted_guard
      if: steps.check_command_position.outputs.command_position_ok == 'true' && steps.check_membership.outputs.is_team_member == 'true'
      env:
        ...
        RUNTIME_STAGE_ID: requirements-clarification
        RUNTIME_CAPABILITY: demo.feature-delivery.requirements-clarification@1.0.0
        RUNTIME_WORKFLOW_ID: feature-delivery-requirements-clarification
...
engine:
  id: copilot
  agent: feature-delivery-requirements-clarification
  max-continuations: 1
  concurrency:
    group: gh-aw-copilot-feature-delivery-requirements-clarification
network: {}
tools:
  github:
    toolsets: [issues]
...
skills:
  - .github/skills/feature-delivery-requirements-clarification
...
# Authorized Feature Delivery requirements clarification
...
Use `feature-delivery-requirements-clarification` and `authority-refusal`. Produce one target-free advisory comment containing a closed feature brief, stable `FD-AC-NNN` acceptance criteria, explicit scope, assumptions, dependencies, and unresolved questions.
