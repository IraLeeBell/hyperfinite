...
description: Stages a target-free bounded patch from the reserved App Modernization implementation stage.
on:
  slash_command:
    name: app-modernization-implementation
  ...
  steps:
    ...
    - name: Build the deterministic guard
      if: steps.check_command_position.outputs.command_position_ok == 'true' && steps.check_membership.outputs.is_team_member == 'true'
      run: npm run build
    - name: Authorize implementation before model execution
      id: trusted_guard
      if: steps.check_command_position.outputs.command_position_ok == 'true' && steps.check_membership.outputs.is_team_member == 'true'
      env:
        ...
        RUNTIME_CAPABILITY: demo.app-modernization.implementation@1.0.0
        RUNTIME_WORKFLOW_ID: app-modernization-implementation
        RUNTIME_DEMO_PROJECT_ID: app-modernization
        RUNTIME_STAGE_ID: implementation
    ...
    - name: Transfer the trusted execution authorization
      ...
      with:
        name: app-modernization-implementation-authorization-${{ github.run_id }}-${{ github.run_attempt }}
...
engine:
  id: copilot
  agent: app-modernization-implementation
  max-continuations: 1
  concurrency:
    group: gh-aw-copilot-app-modernization-implementation
network: {}
tools:
  github: false
skills:
  - .github/skills/app-modernization-implementation
...
safe-outputs:
  ...
  jobs:
    stage-implementation-patch:
      description: Validate one closed target-free App Modernization patch and stage its trusted delivery handoff.
      runs-on: ubuntu-slim
      ...
      inputs:
        planning_artifact_digest:
          description: Exact migration-plan artifact digest from the signed model context.
          required: true
          type: string
      ...
      steps:
        ...
        - name: Transfer the signed trusted execution bundle
          ...
          uses: actions/upload-artifact@v7
          with:
            name: app-modernization-implementation-bundle-${{ github.run_id }}-${{ github.run_attempt }}
        ...
        - name: Invoke the trusted execution delivery service
          ...
          env:
            ...
            TRUSTED_EXECUTION_ARTIFACT_NAME: app-modernization-implementation-bundle-${{ github.run_id }}-${{ github.run_attempt }}
...
# Authorized App Modernization implementation

Use only the signed activation context, migration-plan digest, execution grant, and logical slots for this exact run. Use `app-modernization-implementation` and `authority-refusal`. Call `stage_implementation_patch` exactly once with one closed target-free patch.

The signed target-free planning context is `${{ needs.pre_activation.outputs.model_execution_context_json }}`.

Trusted code alone maps slots to exact paths, validates the complete diff, runs fixed offline commands, and stages draft-pull-request delivery. Never select or emit repository paths, branches, commits, pull requests, Project items, routes, capabilities, credentials, retries, commands, or effects. Do not use network, MCP, shell, packages, lifecycle scripts, or secrets. Do not mark ready, approve, dismiss, merge, deploy, publish, or claim completion.
